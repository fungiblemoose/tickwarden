...
| 2026-06-03 | **capacity cliff matches the budget math** | sim-12 spread bots: 5/6 = 40/45 ms (hold 20 TPS), 7 = 86 ms (drops to 12 TPS). Linear fit (~8 ms/player) predicted the 50 ms budget breaks at ~6.25 — measured 6 holds, 7 breaks. Linearity holds to the cliff, then falls off hard. The box holds ~6 spread players at sim-12 (far more clustered). | `loadtest -load player` sweep on the reference box |
| 2026-06-03 | **player count is linear; clustered ≈ one bubble** | spread bots at sim-12: 1/2/3/4 → 7.1/16.2/22.3/31.6 ms ≈ +8 ms per player (linear → validates the `players ×` term). But 3 *stacked* bots = 7.8 ms ≈ ONE player — a cluster shares one sim bubble. So `-clustered` now treats a group as ~N/3 effective areas (was N/2). | `loadtest -load player` (spread vs stacked) on the reference box |
